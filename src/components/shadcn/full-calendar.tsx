...
const monthEventVariants = cva('w-full', {
  variants: {
    variant: {
      default: 'bg-primary',
      blue: 'bg-blue-500',
          ...
          >
            ...
            <span
              className={cn(
                ...
                isToday(date) && 'size-6 rounded-full bg-primary text-primary-foreground'
              )}
            >
              {format(date, 'd')}
            </span>
...
const CalendarMonthView = () => {
  ...
  return (
    <div className='relative flex h-full flex-col'>
      ...
      {hoveredEvent && (
        <div
          className='absolute z-50 rounded bg-black px-2 py-1 text-xs text-white shadow-md transition-opacity duration-200'
          style={{
            left: `${hoveredEvent.x + 10}px`,
            top: `${hoveredEvent.y - 30}px`, // Décalage vers le haut
            opacity: hoveredEvent ? 1 : 0,
            pointerEvents: 'none', // Évite les bugs de survol
          }}
        >
          {hoveredEvent.title}
        </div>
      ...
      <div className='-mt-px grid flex-1 auto-rows-fr grid-cols-7 gap-px overflow-hidden p-px'>
        {monthDates.map((_date) => {
          const currentEvents = events.filter(
            (event) =>
              isSameDay(event.start, _date) || (event.start < _date && event.end >= _date)
          )
          ...
          return (
            <div
              key={_date.toString()}
              className={cn(
                'relative flex flex-col gap-1 p-2 text-sm text-muted-foreground ring-1 ring-border',
                !isSameMonth(date, _date) && 'text-muted-foreground/50'
              )}
              style={{
                minHeight: `${Math.max(currentEvents.length * 20, 40)}px`,
              }}
            >
              {/* Jour du mois */}
              <span
              ...
              {currentEvents.map((event) => {
                ...
                let rowIndex = eventPositions.get(eventId)

                if (rowIndex === undefined) {
                  rowIndex = 0
                  while (usedRows.has(rowIndex)) rowIndex++
                  eventPositions.set(eventId, rowIndex)
                }

                usedRows.add(rowIndex)

                const startIndex = differenceInDays(event.start, startOfMonth(date))
                const endIndex = differenceInDays(event.end, startOfMonth(date))
                const eventSpan = Math.max(endIndex - startIndex + 1, 1)

                return (
                  <div
                    key={event.id}
                    className={cn(
                      'absolute left-0 flex w-full items-center gap-1 bg-opacity-80 px-2 py-1 text-sm',
                      'text-white',
                      monthEventVariants({ variant: event.color }),
                      'overflow-hidden text-ellipsis whitespace-nowrap',
                      startIndex < 0 && 'rounded-l-none',
                      endIndex >= monthDates.length && 'rounded-r-none'
                    )}
                    style={{
                      top: `${rowIndex * 40}px`,
                      gridColumn: `span ${eventSpan}`,
                    }}
                  >
                    {/* Tooltip hover */}
                      ...
                      <span
                        ...
                        onMouseEnter={(e) => {
                          ...
                          setHoveredEvent({
                            ...
                            title: event.title,
                            x: e.clientX,
                            y: e.clientY - 100, // Position au-dessus de la souris
                          })
                        }}
                        onMouseLeave={() => {
